#!/bin/bash
# ============================================================
#  Zoho Robotics — Repository Setup Orchestrator
#  Master script that runs all setup scripts in the correct
#  order based on the target Git platform.
#
#  Usage:
#    bash devops/setup_repository.sh --platform github
#    bash devops/setup_repository.sh --platform zoho
#    bash devops/setup_repository.sh --platform gitlab
#
#  What it does:
...
#    - devops/project.env must exist in the repository
# ============================================================
...
# ── Audit log ─────────────────────────────────────────────────
# All output mirrored to a temp log, then copied to devops/logs/ after repo detection
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo ""
      echo "Usage: bash devops/setup_repository.sh --platform <platform>"
      echo ""
      echo "Platforms:"
      ...
      echo "  bash devops/setup_repository.sh --platform github"
      echo ""
      exit 0
      ;;
    *)
...
if [[ -z "$PLATFORM" ]]; then
  ...
  echo ""
  echo "Usage: bash devops/setup_repository.sh --platform <github|zoho|gitlab>"
...
# ── Locate devops directory ────────────────────────────────────
TOOLS_DIR="$REPO_ROOT/devops"

if [[ ! -d "$TOOLS_DIR" ]]; then
  echo ""
  fail "devops/ directory not found at: $TOOLS_DIR"
  fail "The setup scripts must be in devops/ before running this script."
  echo ""
  echo "  Copy the devops/ directory from the POC repository:"
  echo "    cp -r ~/robotics-dev/robotics-git-automation-poc/devops ."
  echo "    git add devops/"
...
for script in setup_repo_automation.sh add_status_checks.sh setup_repo_hooks.sh; do
  if [[ ! -f "$TOOLS_DIR/$script" ]]; then
    fail "Missing: devops/$script"
    MISSING_SCRIPTS=true
  fi
done
if [[ "$MISSING_SCRIPTS" == "true" ]]; then
  echo ""
  fail "Required scripts missing from devops/ — copy them from the POC repository"
  exit 1
fi
...
if [[ ! -f "$PROJECT_FILE" ]]; then
  fail "devops/project.env not found"
  exit 1
fi
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║   Zoho Robotics — Repository Setup Orchestrator      ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${RESET}"
echo ""
...
if [[ "$GIT_MAJ" -gt "$REQ_MAJ" ]] || \
  ...
  info "Run: bash devops/setup_developer_system.sh"
  PREREQS_OK=false
fi
...
echo -e "    ${CYAN}bash devops/setup_repo_hooks.sh${RESET}"
echo ""
