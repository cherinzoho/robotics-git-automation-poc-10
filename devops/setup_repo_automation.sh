...
if [[ -f ".husky/pre-push" ]]; then
  ...
  # experiment has special handling — no ticket ID required.
  # release is always separate — version number format only.
  # Both are excluded from TICKET_BRANCH_PATTERN to avoid double-matching.
  TICKET_BRANCH_PATTERN=$(echo "$BRANCH_TYPES" | tr ',' '\n' \
    | grep -v "^experiment$" | tr '\n' '|' | sed 's/|$//' )

  # Build example list of all ticket-based types for the error message
  TICKET_TYPES_DISPLAY=$(echo "$BRANCH_TYPES" | tr ',' '\n' \
    | grep -v "^experiment$" | tr '\n' ' ' | xargs)

  # Use a concrete example matching the ticket pattern
...
#
# Ticket types: ${TICKET_TYPES_DISPLAY}
# Ticket ID pattern: ${TICKET_ID_PATTERN}
PATTERN="^(${TICKET_BRANCH_PATTERN})/${TICKET_ID_PATTERN}-[a-z0-9-]+\$|^release/[0-9]+\\.[0-9]+\\.[0-9]+\$|^experiment/[a-z0-9][a-z0-9-]*\$"
...
if ! echo "\$branch" | grep -qE "\$PATTERN"; then
  ...
  echo "  Valid formats:"
  echo ""
$(echo "$BRANCH_TYPES" | tr ',' '\n' | grep -v "^experiment$" | while read -r t; do
  echo "  echo "    \$t/${EXAMPLE_TICKET}-short-description""
done)
  echo "    release/1.4.0               ← version number, no ticket ID"
  echo "    experiment/topic-name       ← free-form, no ticket ID"
  echo ""
  echo "  Ticket ID format: ${TICKET_ID_PATTERN}  (e.g. ${EXAMPLE_TICKET})"
  echo "  Ticket types:     ${TICKET_TYPES_DISPLAY}"
  echo ""
  exit 1
fi
...
if [[ -f ".github/workflows/pr-validation.yml" ]]; then
  ...
  # Branch type pattern for ticket branches — exclude experiment (has its own rule)
      ...
      - name: Validate branch name
        run: |
          ...
          if echo "\$branch" | grep -qE "\$PATTERN"; then
            ...
            echo ""
            echo "Valid formats:"
            echo "  type/${EXAMPLE_TICKET_ID}-short-description"
            echo "  release/1.4.0"
            echo "  experiment/topic-name"
            echo ""
            echo "Ticket types: $(echo "$BRANCH_TYPES" | tr ',' '\n' | grep -v '^experiment$' | tr '\n' ' ')"
            exit 1
          fi
